...
AngularCacheObject3d::computeDistancesAndSort(
    const std::vector<McVec3f> & coords1,
    const std::vector<McVec3f> & origCoords1,
    const std::vector<McVec3f> & directions1,
    const double              maxDistance,
    const double              minAngleForOptMatching)
{
    const std::vector<McVec3f> & coords2 = this->mCoords;
    const std::vector<McVec3f> & directions2 = this->mDirections;
    ...
    for ( int i=0; i<numCoords1; ++i ) {
        for ( int j=0; j<numCoords2; ++j ) {
            ...
            if ( dist2 < maxDistance2 ) {
                ...
                {
                    pointCorrespondence.push_back(McVec2i(i,j));
                    pointDist2.push_back(dist2);
                }
            }
        }
    }
    ...
    for ( size_t i=0; i<sortedDist2List.size(); ++i ) {
        sortedDist2List[i] = i;   
    }
...
AngularCacheObject3d::getNextMatchingPair(
    ...
    double & dist2)
{
    if ( currListIndex < static_cast<int>(sortedDist2List.size()) ) {
        int ix1, ix2;
        do {
        ...
        } while ( currListIndex < static_cast<int>(sortedDist2List.size()) );
    }
