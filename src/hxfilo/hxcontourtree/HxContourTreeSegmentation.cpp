...
HxContourTreeSegmentation::calculatePlot()
{
    ...
    std::vector<mculong> sortedNodeIdx;
    ...
    std::vector<float> maxValuesOfComponent(sortedNodeIdx.size());
    std::vector<mculong> sizeOfComponent(sortedNodeIdx.size());
    ...
    for (size_t i = 0; i < sortedNodeIdx.size(); ++i)
    {
        if (i % 100 == 0)
        {
        ...
        std::vector<mclong> ids;
        std::vector<float> values;
        std::vector<mculong> sizes;
        std::vector<bool> mergedBecauseOfSize;
        size_t numberOfMerges = 0;
        for (size_t j = 0; j < sortedNeighborIds.size(); ++j)
        {
            const mclong neighbor = m_mesh->getNodeIdx(neighbors[sortedNeighborIds[j]]);
            const mclong component = setUnion.findSetId(neighbor);

            if (!std::binary_search(ids.begin(), ids.end(), component))
            {
                ids.push_back(component);
                values.push_back(maxValuesOfComponent[component]);
                if (j == 0)
                {
                    sizes.push_back(sizeOfComponent[component] + 1);
                }
                else
                {
                    sizes.push_back(sizeOfComponent[component]);
                }
                mergedBecauseOfSize.push_back(false);
        ...
        {
            for (size_t k = j + 1; k < sizes.size(); k++)
            {
                if (sizes[k] < sizes[j])
                {
                    std::swap(values[j], values[k]);
                    std::swap(sizes[j], sizes[k]);
                }
            }
        }
        ...
        for (size_t j = 0; j < values.size(); j++)
        {
            for (size_t k = j + 1; k < values.size(); k++)
            {
                if (values[k] < values[j])
                {
                    std::swap(values[j], values[k]);
                    std::swap(sizes[j], sizes[k]);
                    std::swap(mergedBecauseOfSize[j], mergedBecauseOfSize[k]);
                }
            }
        }
        ...
        for (size_t j = 0; j < sizes.size() - 1; ++j)
        {
            if ((!mergedBecauseOfSize[j]) && (numberOfMerges < sizes.size() - 1))
            {
...
HxContourTreeSegmentation::computeSegmentation(const float persistenceValue, const mculong minNumberVoxels, AugmentedContourTree* augmentedJoinTree)
{
    ...
    std::vector<float> maxValuesOfComponent(sortedNodeIdx.size());
    std::vector<mculong> maxIdOfComponent(sortedNodeIdx.size());
    std::vector<mculong> sizeOfComponent(sortedNodeIdx.size());
...
HxContourTreeSegmentation::sortNeighbors(
    const std::vector<mculong>& neighbors,
    ...
    std::vector<mculong>& sortedNeighborIds)
{
    ...
    for (size_t i = 0; i < sortedNeighborIds.size(); ++i)
    ...
    SetUnionDataStructure& setUnion)
{
    ...
    std::vector<mcuint32> mapToLabel;
    ...
    for (size_t i = 0; i < neighbors.size(); ++i)
    {
        ...
        {
            int arrayId = -1;
            for (size_t j = 0; j < mapToLabel.size(); ++j)
            {
            ...
            if (arrayId == -1)
            {
                ...
                mapToLabel.push_back(label);
                labelCounter.push_back(0);
            }

            labelIds.push_back(arrayId);
        }
        else
        {
            labelIds.push_back(-1);
        }
    }

    std::vector<mculong> numNeighborsWithEqualLabel(neighbors.size());
    for (size_t i = 0; i < numNeighborsWithEqualLabel.size(); ++i)
    {
        sortedNeighborIds[i] = i;
    ...
    SetUnionDataStructure& setUnion)
{
    ...
    std::vector<mclong> oldLabelToNewLabel(setUnion.getNumElements(), -1);

    HxParamBundle* materials = m_outputLabelField->getParameters().getMaterials();
    materials->removeAll();
    materials->insert(new HxParamBundle("Exterior"), 0);
    ...
    for (mculong i = 0; i < size; ++i)
    {
        ...
        if (label > 0)
        {
            ...
            if (oldLabelToNewLabel[componentLabel] < 0)
            {
                ...
                QString mat = QString("Material%1").arg(QString::number(numLabels));
                materials->insert(new HxParamBundle(mat), 0);
                ++numLabels;
            }
...
{
    m_plot->autoUpdate(0);
    PzCurve* curve = m_plot->putData("Number of segments", numberOfPlotPoints, persistanceValues.data(), numberOfSegments.data());
...
HxContourTreeSegmentation::fastContourTreeSegmentation(const float persistenceValue, const mculong minNumberVoxels)
{
    ...
    for (size_t i = 0; i < saddles.size(); i++)
    {
        ...
        tuples.push_back(McVec2d(double(value), double(meshVertexId)));
    }
    ...
    std::vector<float> maxValuesOfComponent = m_maxValuesOfComponentFinestSegmentation;
    std::vector<mculong> sizeOfComponent = m_sizeOfComponentFinestSegmentation;
