...
void 
AngularPointRepresentation3d::setCoords(const std::vector<McVec3f> & coords)
{
    this->mCoords = coords;
...
void
AngularPointRepresentation3d::setOrigCoords(const std::vector<McVec3f> & origCoords)
{
    this->mOrigCoords = origCoords;
}

void 
AngularPointRepresentation3d::setDirections(const std::vector<McVec3f> & directions)
{
        ...
        Transformation                * pointSet2Transform) const
{
    ...
    const std::vector<McVec3f> & coords2 = simplePointSet2->getCoords();
    
    // compute transformation
    mcAlignPointSets(*(McMat4f*)&pointSet2Transform->getTransformation3d(), 
                     &mCoords[0], 
                     &coords2[0], 
                     pointMatching->getRefPoints().data(),
                     pointMatching->getQueryPoints().data(),
        ...
        std::vector<McVec2i>                 & corrGraph) const
{
    ...
    corrGraph.reserve(numPoints1*numPoints2);
    for ( int i=0; i<numPoints1; i++ ) {
        for ( int j=0; j<numPoints2; j++ ) {
            corrGraph.push_back(McVec2i(i,j));
        }
    }
}
        ...
        std::vector<McBitfield>              & connected) const
{
    ...
    for ( size_t i=0; i<corrGraph.size(); i++ ) {
        for ( size_t j=i+1; j<corrGraph.size(); j++ ) {
...
{
    const std::vector<std::vector<double> > & distMatrix1 = this->getDistMatrix();
    const std::vector<std::vector<double> > & distMatrix2 = otherPoints->getDistMatrix();
    ...
    const std::vector<McVec3f> & directions1 = this->getDirections();
    const std::vector<McVec3f> & directions2 = otherPointsA->getDirections();
						 ...
						 double & squaredEdgeWeight) const
{
    ...
    const std::vector<McVec3f> & coords2 = angularCacheObject->getCoords();
    const std::vector<McVec3f> & dirs2   = angularCacheObject->getDirections();
